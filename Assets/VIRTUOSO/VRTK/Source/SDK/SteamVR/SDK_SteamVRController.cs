...
namespace VRTK
{
...
#endif
    {
        ...
        protected Dictionary<uint, SteamVR_Behaviour_Pose> cachedSteamVRPoseObjectsByIndex = new Dictionary<uint, SteamVR_Behaviour_Pose>();
#else
        ...
        [NonSerialized]
        protected Dictionary<ButtonTypes, SteamVR_Action_Boolean> buttonTouchToSteamVRMapping = new Dictionary<ButtonTypes, SteamVR_Action_Boolean>();

        /// <summary>
        /// SteamVR_Input scriptable objects aren't always ready when the rest of the code is, so
        /// the dictionaries need to be populated after startup.
        /// </summary>
        protected void InitializeButtonMapping()
        {
            ...
            buttonPressToSteamVRMapping.Add(ButtonTypes.ButtonTwo, SteamVR_Actions.naturalistic_y);
            buttonPressToSteamVRMapping.Add(ButtonTypes.Grip, SteamVR_Actions.naturalistic_grab);
            buttonPressToSteamVRMapping.Add(ButtonTypes.Trigger, SteamVR_Actions.naturalistic_trigger);
            buttonPressToSteamVRMapping.Add(ButtonTypes.Touchpad, SteamVR_Actions.naturalistic_touchpad_click);

            buttonTouchToSteamVRMapping.Add(ButtonTypes.ButtonOne, SteamVR_Actions.naturalistic_touch_x);
            buttonTouchToSteamVRMapping.Add(ButtonTypes.ButtonTwo, SteamVR_Actions.naturalistic_touch_y);
            buttonTouchToSteamVRMapping.Add(ButtonTypes.Grip, SteamVR_Actions.naturalistic_touch_grab);
            buttonTouchToSteamVRMapping.Add(ButtonTypes.Trigger, SteamVR_Actions.naturalistic_touch_trigger);
            buttonTouchToSteamVRMapping.Add(ButtonTypes.Touchpad, SteamVR_Actions.naturalistic_touch_touchpad);
#endif
        }
#endif
        ...
        public override GameObject GetControllerByIndex(uint index, bool actual = false)
        {
            ...
            if (index < uint.MaxValue)
            {
                ...
                if (sdkManager != null)
                {
                    if (cachedLeftTrackedObject != null && (uint)cachedLeftTrackedObject.inputSource == index)
                    {
                        return (actual ? sdkManager.loadedSetup.actualLeftController : sdkManager.scriptAliasLeftController);
                    }

                    if (cachedRightTrackedObject != null && (uint)cachedRightTrackedObject.inputSource == index)
                    {
                        return (actual ? sdkManager.loadedSetup.actualRightController : sdkManager.scriptAliasRightController);
                    }
                }

                SteamVR_Behaviour_Pose currentTrackedObjectByIndex = VRTK_SharedMethods.GetDictionaryValue(cachedSteamVRPoseObjectsByIndex, index);
        ...
        public override Transform GetControllerOrigin(VRTK_ControllerReference controllerReference)
        {
            ...
            if(poseObject != null)
            {
                return (poseObject.origin != null ? poseObject.origin : poseObject.transform.parent);
            }
#else
        ...
        public override GameObject GetControllerLeftHand(bool actual = false)
        {
            ...
            if (controller == null && actual)
            {
                ...
                {
                    if(steamVrBehavior.inputSource == SteamVR_Input_Sources.LeftHand)
                    {
                        controller = steamVrBehavior.gameObject;
                    }
                }
#else
        ...
        public override GameObject GetControllerRightHand(bool actual = false)
        {
            ...
            if (controller == null && actual)
            {
                ...
                foreach (SteamVR_Behaviour_Pose steamVrBehavior in possibleHands)
                {
                    if (steamVrBehavior.inputSource == SteamVR_Input_Sources.RightHand)
                    {
                        controller = steamVrBehavior.gameObject;
                    }
                }
        ...
        public override Vector3 GetVelocity(VRTK_ControllerReference controllerReference)
        {
            ...
            {
                return poseActions[0].GetVelocity(controllerReference.actual.GetComponent<SteamVR_Behaviour_Pose>().inputSource);
            }
            else
            {
                return Vector3.zero;
        ...
        public override Vector3 GetAngularVelocity(VRTK_ControllerReference controllerReference)
        {
            ...
            if (poseActions.Length > 0)
            {
                return poseActions[0].GetAngularVelocity(controllerReference.actual.GetComponent<SteamVR_Behaviour_Pose>().inputSource);
            }
            else
            {
                return Vector3.zero;
        ...
        public override Vector2 GetButtonAxis(ButtonTypes buttonType, VRTK_ControllerReference controllerReference)
        {
            ...
            switch (buttonType)
            {
                case ButtonTypes.Touchpad:
                    return SteamVR_Actions.naturalistic_touchpad.GetAxis(controllerInputSource);
                default:
        ...
        public override bool GetControllerButtonState(ButtonTypes buttonType, ButtonPressTypes pressType, VRTK_ControllerReference controllerReference)
        {
            ...
            SteamVR_Input_Sources controllerInputSource = controllerReference.actual.GetComponent<SteamVR_Behaviour_Pose>().inputSource;
            if(buttonPressToSteamVRMapping.ContainsKey(buttonType))
            {
                SteamVR_Action_Boolean buttonAction = buttonPressToSteamVRMapping[buttonType];
                if (buttonAction != null) {
                    switch (pressType)
                    {
                        case ButtonPressTypes.Press:
                            return buttonAction.GetState(controllerInputSource);
                        case ButtonPressTypes.PressDown:
                            return buttonAction.GetStateDown(controllerInputSource);
                        case ButtonPressTypes.PressUp:
                            return buttonAction.GetStateUp(controllerInputSource);
                    }
                }
            }

            if (buttonTouchToSteamVRMapping.ContainsKey(buttonType))
            {
                SteamVR_Action_Boolean buttonAction = buttonTouchToSteamVRMapping[buttonType];
                if (buttonAction != null)
                {
                    switch (pressType)
                    {
...
#endif
        {
            ...
            else
            {
                ...
                SteamVR_Behaviour_Pose trackedObject = controller.GetComponent<SteamVR_Behaviour_Pose>();
                 if (trackedObject != null)
                {
                    VRTK_SharedMethods.AddDictionaryValue(cachedSteamVRPoseObjectsByGameObject, controller, trackedObject, true);
                    VRTK_SharedMethods.AddDictionaryValue(cachedSteamVRPoseObjectsByIndex, (uint)trackedObject.inputSource, trackedObject, true);
                }
#else
